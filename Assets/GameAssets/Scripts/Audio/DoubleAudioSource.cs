...
public class DoubleAudioSource : MonoBehaviour {
    ...
    private static float volumeScaler = 1;
    ...
    AudioSource current;
    public void upgdateVolume() {
        volumeScaler = AudioManager.musicVolume * AudioManager.masterVolume;
        if (!fading) {
            current.volume = volumeScaler;
        }
    }
    ...
    IEnumerator Fade(AudioClip playMe,
        ...
        if (delay_before_crossFade > 0) {
            yield return new WaitForSecondsRealtime(delay_before_crossFade);
        }
    ...
    IEnumerator fadeSource(AudioSource sourceToFade, float startVolume, float endVolume, float duration) {
        ...
        while (true) {
            ...
            float volume = Mathf.Clamp01(Mathf.Lerp(startVolume,
                                                            endVolume,
                                                            elapsed / duration));
            sourceToFade.volume = volume * volumeScaler;
